...
export async function removeBackground(formData: FormData) {
  ...
  try {
    ...
    rbFormData.append('image_file', file); rbFormData.append('size', 'auto');
    const res = await fetch('https://api.remove.bg/v1.0/removebg', { method: 'POST', headers: { 'X-Api-Key': apiKey }, body: rbFormData })
    if (!res.ok) throw new Error('AI Service Busy');
    return { data: `data:image/png;base64,${Buffer.from(await res.arrayBuffer()).toString('base64')}` }
...
export async function updateProfile(formData: FormData) {
  ...
  const currentLogo = formData.get('current_company_logo_url') as string
  ...
  const { error } = await supabase.from('profiles').update({
    ...
    company_logo_url: newLogo || currentLogo,
    social_links,
