...
export default function SplashPage({ profile }: { profile: any }) {
  ...
  return (
    <div className="min-h-screen w-full bg-[#050505] flex justify-center items-center p-0 md:p-4 font-sans overflow-hidden select-none">
      ...
      {/* VIEWPORT */}
      <div className="relative h-screen md:h-[850px] w-full max-w-[430px] flex flex-col text-white shadow-[0_0_120px_rgba(0,0,0,1)] md:rounded-[4rem] border-0 md:border-[12px] border-zinc-900 bg-black overflow-hidden group">
        ...
        <div className="relative z-30 w-full px-8 pt-16 flex flex-col items-center text-center">
            {profile.company_logo && (
              <img 
                ...
                className="h-20 w-auto object-contain mb-6 filter drop-shadow-[0_15px_30px_rgba(0,0,0,0.8)]" 
                alt="Brokerage" 
              />
            )}
            ...
            {/* OFFICIAL BRANDFETCH ICONS - Mixed Case Restored, Logo Size Restored */}
               ...
               {profile.social_links && Object.entries(profile.social_links).map(([platform, url]) => {
                 ...
                 return (
                   <a 
                     ...
                     href={url as string} 
                     target="_blank" 
                     className="transition-all duration-300 hover:scale-125 hover:drop-shadow-[0_0_15px_rgba(0,174,239,0.5)]"
                   >
                     <img 
                        src={`https://cdn.brandfetch.io/${domain}/icon`} 
                        className="w-7 h-7 object-contain brightness-[100] grayscale opacity-80 hover:opacity-100 hover:brightness-100 hover:grayscale-0 transition-all duration-300"
                        alt={platform}
