...
import { ExternalLink, Instagram, Facebook, Youtube, Linkedin, MessageSquare } from 'lucide-react'
...
export default function SplashPage({ profile }: { profile: any }) {
  ...
  // Case-insensitive Icon Picker
  const getIcon = (network: string) => {
    const net = network.toLowerCase();
    if (net.includes('instagram')) return <Instagram size={20} />;
    if (net.includes('facebook')) return <Facebook size={20} />;
    if (net.includes('youtube')) return <Youtube size={20} />;
    if (net.includes('linkedin')) return <Linkedin size={20} />;
    if (net.includes('zillow')) return <ZillowIcon />;
    if (net.includes('whatsapp')) return <WhatsAppIcon />;
    if (net.includes('tiktok')) return <MessageSquare size={20} />; // Placeholder for TikTok
  ...
  return (
    <main className="relative h-screen w-full bg-[#050505] overflow-hidden flex flex-col font-sans">
      ...
      {/* 1. BACKGROUND VIDEO (True Black + Glass Layer) */}
      <div className="absolute inset-0 z-0 bg-black">
        <video autoPlay loop muted playsInline key={profile.video_bg_url} className="h-full w-full object-cover opacity-50">
          <source src={profile.video_bg_url} type="video/mp4" />
        </video>
        {/* Subtle Gradient to protect readability */}
      ...
      <div className="relative z-30 flex flex-col items-center pt-12 px-6 text-center">
        ...
        <div className="h-16 w-full flex items-center justify-center mb-6">
          {profile.company_logo && (
            <img src={profile.company_logo} alt="Brokerage" className="max-h-full max-w-[180px] object-contain drop-shadow-2xl" />
          )}
        </div>

        {/* AGENT NAME (Sleek Typography) */}
        <h1 className="text-3xl font-light tracking-[0.2em] text-white uppercase mb-4 drop-shadow-md">
          {profile.agent_name}
        </h1>

        {/* SOCIAL ICONS (Dynamic & Polished) */}
      ...
      {/* 3. THE AGENT (The Floor Layout) */}
      {/* We use translate-y to sit him flush on the dock without cutting feet */}
      <div className="absolute inset-x-0 bottom-0 z-10 flex flex-col items-center pointer-events-none">
        <div className="relative transform translate-y-[20px] md:translate-y-[40px]">
          {profile.avatar_url && (
            <img src={profile.avatar_url} alt="Agent" className="max-h-[82vh] w-auto object-contain drop-shadow-[0_0_80px_rgba(0,0,0,0.9)]" />
          )}
        </div>
      </div>

      {/* 4. PRIMARY CTA (Floating Glass Dock) */}
      <div className="absolute bottom-28 inset-x-0 z-40 px-6 flex justify-center">
        <a href={profile.cta_url} className="group w-full max-w-[340px] rounded-2xl bg-white/5 border border-white/10 backdrop-blur-xl p-5 flex items-center justify-between pointer-events-auto hover:bg-white/10 transition-all active:scale-95">
          <span className="text-[10px] font-black uppercase tracking-[0.4em] text-white pl-4">
            {profile.cta_text || 'Book Consultation'}
          </span>
          <div className="bg-[#00AEEF] p-2 rounded-lg shadow-[0_0_15px_rgba(0,174,239,0.4)]">
             <ExternalLink size={16} className="text-white" />
          </div>
        </a>
      </div>

      {/* 5. LEGAL FOOTER (Required Signatures) */}
      <footer className="absolute bottom-0 inset-x-0 z-50 px-8 py-8">
        <div className="max-w-md mx-auto grid grid-cols-3 items-center">
          ...
          <div className="flex flex-col items-end gap-1">
             ...
             {/* Lynx signature icon placeholder */}
             <div className="w-5 h-2 bg-[#00AEEF]/40 rounded-full" />
          </div>
        </div>
      </footer>
    </main>
