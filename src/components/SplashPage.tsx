...
export default function SplashPage({ profile }: { profile: any }) {
  ...
  const isShort = ratio < 1;  // Vita Vea / Square
  ...
  return (
    <div className="min-h-screen w-full bg-[#050505] flex justify-center items-center p-0 md:p-4 font-sans overflow-hidden">
      <div className="relative h-screen md:h-[850px] w-full max-w-[430px] flex flex-col text-white shadow-2xl md:rounded-[3.5rem] border-0 md:border-[12px] border-zinc-900 bg-black overflow-hidden">
        ...
        {/* --- LAYER 2: THE AGENT (ABSOLUTE BOTTOM ANCHOR) --- */}
          ...
          {profile.avatar_url && (
            <img 
              ...
              className={`
                ...
                ${isTall ? "scale-[1.4] mb-[-5%]" : ""} 
                ${isShort ? "mb-[220px] scale-[0.95]" : "mb-[180px] scale-[1.1]"}
              `}
              style={{ 
        ...
        <div className="relative z-30 w-full px-8 pt-12 flex flex-col items-center text-center">
            ...
            <h1 className="text-4xl font-black italic uppercase tracking-tighter text-white drop-shadow-2xl">
              {profile.agent_name}
            </h1>
            ...
            <div className="flex justify-center gap-6 mt-4">
               {profile.social_links && Object.entries(profile.social_links).slice(0, 4).map(([platform, url]) => {
                 ...
                 return (
                   <a key={platform} href={href} target="_blank" rel="noreferrer" className="transition-all duration-300 text-white/50 hover:text-[#00AEEF]">
        ...
        <div className="relative z-30 mt-auto w-full px-8 pb-10 pt-20 bg-gradient-to-t from-black via-black/90 to-transparent flex flex-col items-center gap-6">
            <a 
